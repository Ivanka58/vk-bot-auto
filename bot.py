...
def choose_category(message):
    ...
    if user_data.get(chat_id, {}).get('state') != 'category':
        return
    cat = 'usual' if 'Обычные' in message.text else 'large'
    user_data[chat_id]['category'] = cat
    user_data[chat_id]['state'] = 'photo'
    bot.send_message(chat_id, "📷 Отправь фото (до 10 шт.). Когда закончишь — нажми кнопку.", reply_markup=photo_kb())
...
def handle_photo(message):
    ...
    bot.send_message(chat_id, f"📷 Фото {len(photos)}/10. Можешь ещё или нажать «Закончить отправку фото ✅»", reply_markup=photo_kb())
...
def handle_text(message):
    ...
    preview = f"📋 Предпросмотр:\n\n{message.text}\n\n📷 Фото: {len(user_data[chat_id]['photos'])}"
...
def confirm_send(message):
    ...
    print(f"[CONFIRM] Отправка: {len(data['photos'])} фото, текст: {data['text'][:60]}")
    ...
    bot.send_message(chat_id, "⏳ Отправляю в группы ВК...")
    try:
